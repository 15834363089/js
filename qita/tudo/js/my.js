    ...
    $.each(data, function (index, val) {
        if (state == 'wait') {//如果是未完成的
            if (val.isDone == 0) {
                ...
                str += "<li id='" + index + "'  style='background:" + val.color + "'><time>" + getYear(val.time) + "<br><span>" + gettime(val.time) + "</span></time> <p>" + val.con + "</p> <span   class='iconfont "+className+" '>&#xe662;</span><div class='right'>完成</div></li>";
            }
        }
        else if (state == 'done') {//如果是已完成的状态
            if (val.isDone == 1) {
                str += "<li id='" + index + "'   style='background:#CCCCCC'><time>" + getYear(val.time) + "<br><span>" + gettime(val.time) + "</span></time> <p>" + val.con + "</p><span class='iconfont '>&#xe662;</span><div class='del' style='background:#999;'>删除</div></li>";
            }
        }
    })
    $('.content ul').html(str);
